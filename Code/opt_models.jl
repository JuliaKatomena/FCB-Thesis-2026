...
function reserve_RI_NI(NI_up, NI_down, Cf_Rup, Cf_Rdown)
    ...
    all_vars = all_variables(model)  # returns an array of all variables
    println("nb of variables ", length(all_vars))
    println("nb of constraints ", sum(num_constraints(model, F, S) for (F, S) in list_of_constraint_types(model)))
    num_bin = count(is_binary, all_vars)
    println("Binary variables: ", num_bin)
...
function TA_bin_GDCA(Pw, nS, prob, Q_up, Q_down,  rf_up, rf_down,
    ...
    for iter in 1:max_iter
        ...
        #println("zk_val_up: ", zk_val_up)
        #println("ppBsup: ", JuMP.value.(ppB_up).*value.(s_up))
        #println("zk_val_down: ", zk_val_down) 
        #println("ppBinf: ", JuMP.value.(ppB_down).*value.(s_down))

        # println("s_up", value.(s_up))
        # println("z_up", value.(z_up))
        # println("k_up", value.(k_up))
        # println("ppB_up", value.(ppB_up))
        # println("s_up*ppB_up", JuMP.value.(ppB_up).*value.(s_up))
        # println("zk_val_up", zk_val_up)
        # println("s_down", value.(s_down))
        # println("z_down", value.(z_down))
        # println("Imbalance", value.(Imb))
        ...
        # step_norm < tol &&
        #println("Diff Matrix up is", diff_matrix_up)
        #println("Diff Matrix down is", diff_matrix_down)
        ...
        if step_norm < tol && all(abs.(diff_matrix_up) .< tol) && all(abs.(diff_matrix_down) .< tol) && all(value.(t) .< tol)            
            ...
            if any(abs.(diff_matrix_down) .> tol)
                #println("⚠️ Some values in diff_matrix_down exceed tolerance $tol")
            end
            #   println("TA Bin Converged after  $iter iterations")
            
            if step_norm > tol
                println("Step norm is $(step_norm)")
            end
            return iteration_history
        end
        ...
        inv_step = 1.0 / max(step_norm, eps())
        rk = min(inv_step, norm(Lambda_t, 1) + delta_1)
        #println("max step norm:", max(step_norm, eps()))
        if (penalty_t < rk)
